...
- `WithVAPIDTokenTTL` **(Enabled by default)** Caching jwt token + local public key and curve. When preparing request,
...
- `WithLocalSecretTTL` Reuse the local public key and secret if available, or else generate new ones and set
  to `Subscription`, user can save those keys for reuse later. When preparing request, this option improving performance
